...
class Bankers(object):
    ...
    def ExecuteProcess(self,index):
        ...
        self.CalcNeed()
        Requesti = self.need[index]
        for i in range(0,len(Requesti)):
            if (self.avaliable[i] - Requesti[i] < 0):
                return False
        #check END here
        ...
        # YOUR CODE, 2012011364
        for i in range(0,len(Requesti)):
            self.avaliable[i] = self.avaliable[i] - self.need[index][i]
    ...
    def TempSafeCheckAfterRelease(self):
        ...
        procLen = len(self.need)
        resLen = len(self.avaliable)

        while (1):
            Ti = -1
            for i in range(0,procLen):
                needOK = True
                for j in range(0,resLen):
                    if self.need[i][j] > Work[j]:
                        needOK = False
                        break
                if  (tempFinish[i] == False and needOK):
                    Ti = i
                    break
            ...
            for i in range(0,resLen):
                Work[i] = Work[i] + self.allocated[Ti][i]

        # check if safe
        safeFlag = True
        for i in range(0,procLen):
            if (tempFinish[i] == False):
                safeFlag = False
                break
