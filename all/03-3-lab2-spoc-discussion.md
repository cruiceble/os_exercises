...
- 将kernel的代码中intr_enable()中的代码注释掉发现qemu不再退出, 推断是打开中断导致退出。
  因为在代码trap.c中, 我们需要对中断向量表进行初始化。 
...
x=11；
没有差异。在trap.c的trap_dispatch函数中可以看到，lab1支持的中断只包括32,36,21,152,153,46,47。其他都属于default，将调用如下代码：
   if ((tf->tf_cs & 3) == 0) {
        	//cprintf("lalalalala\n");
            print_trapframe(tf);
