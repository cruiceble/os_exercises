...
> 初始化信息在kern_init()函数中 ，首先运行pic_init()进行8259中断控制器的配置，通过汇编的out指令进行。然后通过idt_init()中的SETGATE宏初始化中断向量表，通过`lidt`的汇编命令加载idt。随后对时钟进行初始化，通过clock_init()完成，主要通过汇编的out指令进行  
...
> 在 trap.c 中的函数 trap_dispatch 发现uCore处理的所有外设有：IRQ_TIMER 时钟外设，IRQ_KBD 键盘，IRQ_COM1 串口，IRQ_IDE1 IDE口1，IRQ_IDE2 IDE口2
...
> 　　在每次触发系统调用的时候分别进行统计，每个系统调用开始执行的时候在建好的系统调用表中相应表项次数加一，获取所有调用次数则将记录的各个次数求和。要注意每项计数的初始化。
