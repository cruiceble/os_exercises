...
```
proc.h中定义,一个进程有两个堆栈，一个用户态，一个内核态，切换时由用户态转为内核态，但是内核不是一个单独的进程。区别是不是同一个进程看地址空间是不是同一个，即cr3是不是一样，在proc_run中，切换时不涉及cr3的改变，所以操作系统负责切换的内核不是一个单独的进程。不同的进程，内核空间是一样的，用户空间不一样。如果是内核态发生中断，就不涉及栈的切换
